@props(["categorias","text","type"])

@if ($text!=null)
    @if (strcmp($type,"link")==0)
        <form action="{{ route("producto.categoria") }}" method="POST">@csrf<button class="m-0 p-0 ms-4 px-1 raiz-catego" type="submit" name="idCatego" value="{{ $text }}">{{ $text }}</button></form>
    @elseif (strcmp($type,"buscar")==0)
        <form action="{{ route("producto.buscarproductos") }}" method="POST">@csrf<button class="m-0 p-0 ms-4 px-1 raiz-catego" type="submit" name="catego" value="{{ $text }}">{{ $text }}</button></form>
    @else
        <p class="m-0 p-0 ms-4 raiz-catego">{{$text}}</p>
    @endif
@endif
@foreach ($categorias as $categoria)
    ...
    @else
        <div class="catego">
            <div class="d-flex align-items-center @if (strcmp($type,"link")==0 || strcmp($type,"linkTodo")==0) ps-1 @endif  boton-catego">
                ...
                @if (strcmp($type,"link")==0)
                    <form action="{{ route("producto.categoria") }}" method="POST">@csrf<button class="m-0 p-0 px-1 btn-catego" type="submit" name="idCatego" value="{{ $categoria["id"] }}" data-id="{{ $categoria["id"] }}">{{$categoria['titulo']}}</button></form>
                @elseif (strcmp($type,"buscar")==0)
                    <form action="{{ route("producto.buscarproductos") }}" method="POST">@csrf<button class="m-0 p-0 px-1 btn-catego" type="submit" name="catego" value="{{ $categoria["id"] }}" data-id="{{ $categoria["id"] }}">{{$categoria['titulo']}}</button></form>
                @else
