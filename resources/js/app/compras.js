...
(()=>{
    const ctx = document.querySelector('#gastosChart');
    if (ctx) {
        ...
        const chart=new Chart(ctx, {
            ...
            data: {
                ...
                datasets: chartData.gastosVisibles.map(dataset => ({
                    ...dataset,
        ...
        const checkboxes = selector.querySelectorAll('.year-checkbox').forEach(cb => {
            cb.addEventListener("change",()=>{
                ...
                const datasetsSeleccionados = Array.from(cbSelecteds).map(cb => 
                    chartData.gastos.find(ds => ds.label === cb.value)
                );
    ...
    function actualizarBotonMasAnios(chartData,chart) {
        const btnMostrarMas = document.querySelector('#btnMostrarMasAnios');
        if (btnMostrarMas) {
            const añosOcultos = chartData.gastos.length - chart.data.datasets.length;
...
(()=>{
    document.addEventListener("click", function (event) {
        const collapseEl = document.querySelector("#collapse-filtroFechaTotalCompra");
        if(collapseEl!=null){
            const bsCollapse = bootstrap.Collapse.getInstance(collapseEl);
            if (bsCollapse!=null && collapseEl.classList.contains("show")) {
                if (!collapseEl.contains(event.target) && !document.querySelector('[data-bs-target="#collapse-filtroFechaTotalCompra"]').contains(event.target)) {
                    bsCollapse.hide();
                }
            }
        }
...
(()=>{
    window.addEventListener("load",()=>{
        const btnF=document.querySelector("#btn-filtroTotalCompra-fechaPresonal");
